...
void calc_step_in_cell( struct ParametersCommon *paramsc, struct Parameters1d *params1d, struct DebugInfo *debug_info, double u_left[M], double u_center[M],
                        ...
                        double dt, double h, double u_next[M], int step_number, int n, bool is_pressure_relaxation_now, int number_of_scalars, double curr_time, double *configuration_pressure, double S_center, double S_left, double S_right, int l ) {
    ...
    switch ( paramsc->numerical_method ) {
        ...
        case GODUNOV:
            ...
            break;
        case HLL:
            hll( paramsc, params1d, debug_info, u_left, u_center, u_right, slopes_left, slopes_center, slopes_right, dt, h, u_next, step_number, n, is_pressure_relaxation_now, params1d->number_of_scalars, curr_time, configuration_pressure, S_center, S_left, S_right, l  );
            break;
        case RUSANOV:
...
void calc_sound_velocity( struct DebugInfo *debug_info, const struct ParametersCommon* paramsc, const double v_ncons[M], double* c1, double* c2 ) {
    ...
    if (v_ncons[P_DISP] < 0.0 || v_ncons[P_GAS] < 0.0){
        ...
        if (v_ncons[P_GAS] < 0.0){
            ...
            printf("\ncalc_sound_velocity -> pressure of gas phase is %lf\n", v_ncons[P_GAS]);
        }
        exit(EXIT_FAILURE);
    }
...
// Расчет скоростей звука для обеих фаз по полному вектору примитивных переменных без отладки
// paramsc - структура с основными параметрами вычислительного эксперимента
// v_ncons - вектор примитивных переменных
// c1 - скорость звука в дисперсной фазе
// c2 - скорость звука в газовой фазе
...
double calc_sound_velocity_one_phase( const struct ParametersCommon* paramsc, const double v_ncons[M], const Phase phase ) {
    ...
    if ( phase == GAS_PHASE ){
        if ( v_ncons[P_GAS] < 0.0){
            printf("\ncalc_sound_velocity_one_phase -> pressure of gas phase is negative\n");
            exit(EXIT_FAILURE);
        }
...
ReturnCodes convert_cons_to_noncons( const struct ParametersCommon* paramsc, const double v_cons[M], double v_ncons[M], int number_of_scalars, double S ) {

    double b2 = S - v_cons[B_DISP]; // объемная доля газовой фазы
    ReturnCodes return_code = SUCCESS; // код возврата функции

    v_ncons[B_DISP] = v_cons[B_DISP] / S; // b1, объемная доля дисперсной фазы
...
void convert_noncons_to_cons( const struct ParametersCommon *paramsc, const double v_ncons[M], double v_cons[M], int number_of_scalars, double S ) {

    double b2 = 1 - v_ncons[B_DISP]; // объемная доля газовой фазы 

    v_cons[B_DISP] =S * v_ncons[B_DISP]; // b1, объемная доля дисперсной фазы 
    v_cons[R_DISP] =S * v_ncons[B_DISP] * v_ncons[R_DISP]; // b1 * r1
    v_cons[V_DISP] = v_cons[R_DISP] * v_ncons[V_DISP]; // b1 * r1 * v1
    v_cons[P_DISP] = S * v_ncons[B_DISP] * v_ncons[R_DISP] * ( 0.5 * pow( v_ncons[V_DISP], 2.0 ) + v_ncons[P_DISP] / v_ncons[R_DISP] /
        ( paramsc->g1 - 1.0 ) + paramsc->g1 * paramsc->p01 / ( paramsc->g1 - 1.0 ) / v_ncons[R_DISP] ); // b1 * r1 * E1 
    v_cons[R_GAS] = S * b2 * v_ncons[R_GAS]; // b2 * r2
    ...
    //printf("\n%lf", v_cons[P_DISP]);
    for (int i = 0; i < number_of_scalars; i++)
        v_cons[Z0 + i] = S * v_ncons[Z0 + i] * v_ncons[R_DISP] * v_ncons[B_DISP];
}
...
// v_ncons - вектор примитивных переменных
// flux - рассчитываемый вектор дифференциального "потока"
void diff_flux_ncons( const struct ParametersCommon* paramsc, const double v_ncons[M], array1D* flux, int number_of_scalars, double S) {

    double v_cons[M]; // вектор "консервативных" переменных

    convert_noncons_to_cons( paramsc, v_ncons, v_cons, number_of_scalars, S );

    double b2 = 1 - v_ncons[B_DISP]; // объемная доля газовой фазы
    
    (*flux)[B_DISP] = 0.0; // поток объемной доли дисперсной фазы
    (*flux)[R_DISP] = v_cons[V_DISP]; // поток массы в дисперсной фазе
    (*flux)[V_DISP] = S * v_ncons[B_DISP] * ( v_ncons[R_DISP] * pow( v_ncons[V_DISP], 2.0 ) + v_ncons[P_DISP] ); // поток импульса в дисперсной фазе
    (*flux)[P_DISP] = v_ncons[V_DISP] * ( v_cons[P_DISP] + S * v_ncons[B_DISP] * v_ncons[P_DISP] ); // поток энергии в дисперсной фазе
    (*flux)[R_GAS] = v_cons[V_GAS]; // поток массы в газовой фазе
    (*flux)[V_GAS] = S * b2 * ( v_ncons[R_GAS] * pow( v_ncons[V_GAS], 2.0 ) + v_ncons[P_GAS] ); // поток импульса в газовой фазе
    (*flux)[P_GAS] = v_ncons[V_GAS] * ( v_cons[P_GAS] + S * b2 * v_ncons[P_GAS] ); // поток энергии в газовой фазе
...
// v_cons - вектор "консервативных" переменных
// rhst - неконсервативный вектор правых частей
void rhst_ncons(double h, double splus_l, double sminus_l, double splus_r, double sminus_r, const struct ParametersCommon* paramsс, const double v_ncons[M], array1D* rhst, array1D* additional, int number_of_scalars, double S_center, double S_left, double S_right  ) {
    double v_cons[M]; // вектор "консервативных" переменных
    convert_noncons_to_cons(paramsс, v_ncons, v_cons, number_of_scalars );
    double dSdx; // аппроксимация градиента объемной доли газовой фазы
    calc_dS( splus_l, sminus_l, splus_r, sminus_r, &dSdx, S_center, S_left, S_right, h );
    ...
    (*rhst)[R_DISP] = 0; // ЗСМ в дисперсной фазе
    (*rhst)[V_DISP] = p_i  ; // ЗСИ в дисперсной фазе
    (*rhst)[P_DISP] = p_i * u_i  ; // ЗСЭ в дисперсной фазе
    (*rhst)[R_GAS] = 0; // ЗСМ в газовой фазе
    (*rhst)[V_GAS] =  -p_i  ; // ЗСИ в газовой фазе
    (*rhst)[P_GAS] =  -p_i * u_i ; // ЗСЭ в газовой фазе

    // члены связаннаые с переменным сечением
    (*additional)[B_DISP] = 0; // уравнение компактирования
    (*additional)[R_DISP] = - v_ncons[B_DISP] * v_ncons[R_DISP] * v_ncons[V_DISP] * dSdx / S_center; // ЗСМ в дисперсной фазе
    (*additional)[V_DISP] = - v_ncons[B_DISP] * ( v_ncons[R_DISP] * pow(v_ncons[V_DISP], 2) ) * dSdx / S_center; // ЗСИ в дисперсной фазе
    (*additional)[P_DISP] = - v_ncons[V_DISP] * ( v_cons[P_DISP] +  v_ncons[B_DISP] * v_ncons[P_DISP] ) * dSdx / S_center; // ЗСЭ в дисперсной фазе
    (*additional)[R_GAS] = -v_cons[V_GAS] * dSdx / S_center; // ЗСМ в газовой фазе
    (*additional)[V_GAS] = -  b2 * ( v_ncons[R_GAS] * pow( v_ncons[V_GAS], 2.0 ) ) * dSdx / S_center; // ЗСИ в газовой фазе
    (*additional)[P_GAS] = - v_ncons[V_GAS] * ( v_cons[P_GAS] + b2 * v_ncons[P_GAS] ) * dSdx / S_center ; // ЗСЭ в газовой фазе
...
// Аппроксимация дифференциала dS в правой части
...
// dS2dx - искомая аппроксимация
void calc_dS(double splus_l, double sminus_l, double splus_r, double sminus_r, double* dS2dx, double S_center, double S_left, double S_right,double h  ) {

    *dS2dx = ( ( splus_r * S_center - sminus_r * S_right ) / ( splus_r - sminus_r )
        - ( splus_l * S_left - sminus_l * S_center ) / ( splus_l - sminus_l ) )/h;
    //*dS2dx = (S_right - S_left)/(2*h);
}
