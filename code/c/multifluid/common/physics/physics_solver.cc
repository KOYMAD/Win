...
void physics_solver( struct ParametersCommon *paramsc, struct Parameters1d *params1d, struct Parameters2d *params2d, Direction2d dir, double dt, double *v_ncons, double *v_ncons_next, double *v_ncons_prev, int *number_of_block, double *beta, double *B, double *C, int n, int ignition_flag, int initial_ignition, double S, double time_mom ) {
    ...
    if ( v_ncons[B_DISP] < paramsc->eps_disp_abs )
        if (initial_ignition == 1){
			// если задано, что область должна газить даже без пороха, то не выходим
        }
        else
    ...
    if (paramsc->program_name == ONED2PHC || paramsc->program_name == ONED3PHC){
	    ...
	    calc_right_hand_side_terms( paramsc, params1d, params2d, dir, v_ncons, right_hand_side_terms, number_of_block, M, ignition_flag, initial_ignition, S, time_mom ); // расчет текущего вектора правых частей
        ...
        for ( i_step = 0; i_step < paramsc->substeps_num; i_step++ ) { // цикл интегрирования по времени
	    calc_right_hand_side_terms( paramsc, params1d, params2d, dir, v_ncons, right_hand_side_terms, number_of_block, M2D, ignition_flag, initial_ignition, S, time_mom ); // расчет текущего вектора правых частей
            if (paramsc->numerical_method_physics == EXPLICIT_EULER){
                for ( i_component = 0; i_component < n; i_component++ ) { // цикл по компонентам вектора
                    // явный метод Эйлера
	            v_cons[i_component] = v_cons[i_component] + sub_dt * right_hand_side_terms[i_component];	
    ...
    *C = calc_chemical_reaction(paramsc, v_ncons, initial_ignition);
}
...
void calc_right_hand_side_terms( struct ParametersCommon *paramsc, struct Parameters1d *params1d, struct Parameters2d *params2d, Direction2d dir, double *v_ncons, double *right_hand_side_terms, int *number_of_block, int n, int ignition_flag, int initial_ignition, double S, double time_mom ) {
    ...
    if (paramsc->program_name == ONED2PHC || paramsc->program_name == ONED3PHC ) {
        ...
        double C = calc_chemical_reaction(paramsc, v_ncons, initial_ignition);
	...
	if (initial_ignition == 1 ){ // Если ячейка газит, то вводим параметры газоприхода
	...
	double heat_release;
	heat_release = paramsc->heat_release;
	if ( initial_ignition == 2){
            if (time_mom < 0.3){// Ячейка с другим порохом, более энергичное горение
		C *= 14 * pow((0.3 - time_mom)/0.3, 2);
		heat_release *= 14 * pow((0.3 - time_mom)/0.3, 2);
	}
	// здесь происходит подгон кривой давления вдува под имеющиеся давнные
	if (time_mom < params1d->ignition_time * 0.4)
	{
		mass_input = mass_input * time_mom/(params1d->ignition_time * 0.4);

	}
		if (time_mom > params1d->ignition_time * 0.4)
	{
		mass_input = mass_input * pow((time_mom/(params1d->ignition_time * 0.4)) , 2); //- 1 / 4 * (time_mom - params1d->ignition_time * 0.4)/(params1d->ignition_time * 0.4));

	}
		if (time_mom > params1d->ignition_time * 0.8)
	{
		mass_input = mass_input * pow(((params1d->ignition_time * 0.8) / time_mom) , 6); 

	}
		if (time_mom > params1d->ignition_time )
	{
	    additional_heat = 0.3 * params1d->additional_heat * pow(params1d->ignition_time / time_mom, 4);
        mass_input = 0.3 * params1d->mass_input * pow(params1d->ignition_time / time_mom, 4);
		...
		if (paramsc->ignition_condition == 2){//воспламенение по температуре
		    if (T_gas(paramsc, v_ncons[P_GAS], v_ncons[R_GAS]) > paramsc->T_ignition )
			...
			- ( e_disp(paramsc, v_ncons[P_DISP], v_ncons[R_DISP]) + 0.5 * v_ncons[V_DISP] * v_ncons[V_DISP] + B + beta / v_ncons[R_DISP] + heat_release) * C + additional_heat*mass_input  - heat_transfer);	
		    else
			right_hand_side_terms[P_GAS] = (v_ncons[P_GAS] * compaction_rate - (- friction_force + 0.5 * C * (v_ncons[V_GAS] - v_ncons[V_DISP])) * calc_u_i( paramsc, v_ncons ) 
		...
		if (paramsc->ignition_condition == 1){ // воспламенение по давлению
                    if (v_ncons[P_GAS] > paramsc->p_ignition || initial_ignition == 2)
			right_hand_side_terms[P_GAS] =( v_ncons[P_GAS] * compaction_rate - (- friction_force + 0.5 * C * (v_ncons[V_GAS] - v_ncons[V_DISP])) * calc_u_i( paramsc, v_ncons ) 
			- ( e_disp(paramsc, v_ncons[P_DISP], v_ncons[R_DISP]) + 0.5 * v_ncons[V_DISP] * v_ncons[V_DISP] + B + beta / v_ncons[R_DISP] + heat_release ) * C + additional_heat*mass_input  - heat_transfer);	
		    else
			right_hand_side_terms[P_GAS] =( v_ncons[P_GAS] * compaction_rate - (- friction_force + 0.5 * C * (v_ncons[V_GAS] - v_ncons[V_DISP])) * calc_u_i( paramsc, v_ncons ) 
  ...
  //      right_hand_side_terms[P_GAS] =  -  0.5 * C * (v_ncons[V_GAS] - v_ncons[V_DISP]) * calc_u_i( paramsc, v_ncons ) - ( e_disp(paramsc, v_ncons[P_DISP], v_ncons[R_DISP]) + 0.5 * v_ncons[V_DISP] * v_ncons[V_DISP] + paramsc->heat_release) * C;
    ...
    // Если задано, что область должна газить даже в случая отсутсвия пороха, то эти условия
    if  (initial_ignition == 1){
        right_hand_side_terms[R_DISP] =0;
        right_hand_side_terms[V_DISP] =0;
        right_hand_side_terms[P_DISP] =0;
        right_hand_side_terms[R_GAS] =mass_input;
        right_hand_side_terms[V_GAS] =0;
        right_hand_side_terms[P_GAS] =additional_heat*mass_input;
    }
    }
...
void runge_kutta_predictor_2_nd_order (struct ParametersCommon *paramsc, struct Parameters1d *params1d, struct Parameters2d *params2d, Direction2d dir, double v_cons[M], double right_hand_side_terms[M], double sub_dt, int *number_of_block, int ignition_flag, int initial_ignition, double S, double time_mom ){
    ...
    calc_right_hand_side_terms(paramsc, params1d, params2d, dir, v_ncons_predicted, right_hand_side_from_prediction, number_of_block, M1D, ignition_flag, initial_ignition, S, time_mom);
