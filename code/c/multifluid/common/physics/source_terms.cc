...
double calc_friction_force( struct ParametersCommon *paramsc, double v_ncons[M]) {
    ...
    switch ( paramsc->fric_force_formula_num ) {
        ...
        case HOMENKO:
            ...
            double Sp = 1.38 * 1578 * 1 / (1 - psi) * X * (1 + 2 * lambda * v_ncons[Z0] + 3 * v_ncons[Z0] * v_ncons[Z0] ); // Площадь межфазной поверхности
            ...
            break;
        default:
            printf( "\ncalc_friction_force -> fric_force_formula_num should be 1, 2, 3, 4 or 5 only.\n\n" );
            exit( EXIT_FAILURE );
    }
...
double calc_chemical_reaction(const struct ParametersCommon *paramsc, double v_ncons[M], int initial_ignition){
    ...
    if (paramsc->is_physics && paramsc->is_burning ){
		...
		if (paramsc->ignition_condition == 2){ // воспламенение по температуре
			...
			return - v_ncons[B_DISP] * v_ncons[R_DISP] * paramsc->U_coef / paramsc->e_coef * pow ( v_ncons[P_GAS], paramsc->nu_coef) * 
			       X * (1.0 + 2.0 * v_ncons[Z0] * lambda + 3.0 * mu * v_ncons[Z0] * v_ncons[Z0]) / (1.0 - psi);
		    }
		    else
			return 0.0;									
		    }
