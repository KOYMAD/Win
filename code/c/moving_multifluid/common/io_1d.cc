...
void fill_parameters_1d( FILE *params, struct ParametersCommon* paramsc, struct Parameters1d* params1d ) {
    ...
    printf("%d", params1d->cells_number);
    ...
    // движущееся тело
    fscanf_s( params, "%s", string, MAX_STRING_SIZE ); // заголовок раздела
    fscanf_s( params, "%s %lf", string, MAX_STRING_SIZE,&(params1d->body_left_boundary_x) ); //левая граница движущегося тела
    fscanf_s( params, "%s %lf", string, MAX_STRING_SIZE,&(params1d->body_right_boundary_x) ); //правая граница движущегося тела
    fscanf_s( params, "%s %lf", string, MAX_STRING_SIZE,&(params1d->body_cross_section_divided_to_mass) ); //площадь к массе
    fscanf_s( params, "%s %lf", string, MAX_STRING_SIZE,&(params1d->resistive_pressure) ); //давление форсирования
    // начальные условия
    fscanf_s( params, "%s", string, MAX_STRING_SIZE ); // заголовок раздела
...
void write_solution_1d2phc( struct ParametersCommon *paramsc, struct Parameters1d* params1d, char *output_directory, int cells_number, double *xc, double **v_ncons, int file_number, char *output_filename, double *beta, double *B, double *C, int n, int *ignition_flag, int *initial_ignition, int *status) {
	...
	fprintf( f_out, " %e", B[i] );
        fprintf( f_out, " %e", C[i] );
	fprintf(f_out," %d", ignition_flag[i]);
	fprintf(f_out," %e",T_gas(paramsc, v_ncons[i][P_GAS], v_ncons[i][R_GAS]));
	fprintf(f_out," %d", initial_ignition[i]);      
            //fprintf(f_out," %d", status[i]);
        fprintf( f_out, "\n" );
    }
...
void write_solution_tecplot_header( struct ParametersCommon *paramsc, struct Parameters1d *params1d, int cells_num, FILE *file_to_write, int file_number ) {
	...
	fprintf( file_to_write, "\"Chemical reaction\"\n" );
        /* будет ли продолжаться горение */
	fprintf( file_to_write, "\"ignition flag\"\n" );
	/* температура газовой фазы */
	fprintf( file_to_write, "\"Gas phase temperature\"\n" );
        /* начальное воспламенение */
	fprintf( file_to_write, "\"initial ignition\"\n" );
    }
...
void write_sensors( struct ParametersCommon *paramsc, double **v_ncons, struct TimeMoment *time_mom, FILE *sensors_files[MAX_SENSORS_NUM],int sensors_cells[MAX_SENSORS_NUM], int n, double body_velocity ) {
    ...
    for ( int i_sensor = 0; i_sensor < paramsc->sensors_num; i_sensor++ ) {
        ...
        fprintf_s( sensors_files[i_sensor], "%e\t", body_velocity );
